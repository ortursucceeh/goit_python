...
def show_commands():
    ...
    print("All commands:")
    contact_func = list(key for key, value in handler.items() if hasattr(Book, value[0].__name__))
    notes_func = list(key for key, value in handler.items() if hasattr(Notes, value[0].__name__))
    other_commands = ["show_commands", "show_doc", "show_all_contacts", "show_all_notes", 'filter_folder']
