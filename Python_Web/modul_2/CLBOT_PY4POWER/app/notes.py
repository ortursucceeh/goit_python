...
class Notes(UserDict):
    ...
    def add_note(self, title, keywords, notedata):
        ...
        if str(title) not in self.data:
            record = NoteRecord(title, Keywords(keywords), Notedata(notedata))
    ...
    def show_all_notes(self):
        data = [
            [str(i.title), str(i.keywords), str(i.notedata)] for i in self.data.values()
        ]
        header = ("Title", "Keywords", "Data")
        show_in_console(data, header)
    ...
    def show_notes_with_tag(self, tag):
        data = [
            [str(rec.title), str(rec.keywords), str(rec.notedata)]
    ...
    def sort_notes_by(self, category):
        if category == "1":
            ...
            def key(x):
                return str(x[1].title)

        elif category == "2":

            def key(x):
                return str(x[1].keywords)

        else:

            def key(x):
                return str(x[1].notedata)
