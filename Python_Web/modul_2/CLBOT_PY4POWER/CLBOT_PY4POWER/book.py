...
class Book(UserDict):
    ...
    def add_contact(self, name, phones, email, birthday, address):
        ...
        if str(name) not in self.data:
            record = ContactRecord(
                name, Phones(phones), Email(email), Birthday(birthday), Address(address)
            )
    ...
    def show_contact(self, search_data):
        data = [
            [str(i.name), str(i.phones), str(i.email), str(i.birthday), str(i.address)]
            for k, i in self.data.items()
            if search_data in str(i.name).lower()
    ...
    def add_phone(self, name, phones):
        ...
        if str(name) in self.data:
            ...
            if len_phones < 3:
                ...
                return (
                    f"You entered too much phones. You can add {3 - len_phones} phones"
                )
    ...
    def change_phone(self, name, old_phone, new_phone):
        ...
        if str(name) in self.data:
            ...
            if old_phone[0] in str(record.phones):
                record.phones.value[
                    record.phones.value.index(old_phone[0])
                ] = new_phone[0]
    ...
    def show_birthdays_after(self, daysNumber):
        if self.data:
            ...
            data = [
                ...
                and datetime.strptime(i.birthday.value, "%d.%m.%Y").month
                == check_day.month
            ]
            if data:
                headers = ("Name", "Phones", "Email", "Birthday", "Address")
