...
document = os.path.join(
    ...
    "PY4POWER_Logo_and_documentation",
    "documentation.txt",
)
...
def hello_func():
    print(
        """Hello! I am your personal assistant CLBOT. How can I help you?
...
def start_func():
    ...
    global notes
    if os.path.exists(
        os.path.join(saving_folder, "contact_book.pickle")
    ) and os.path.getsize(os.path.join(saving_folder, "contact_book.pickle")):
    ...
    if (
        os.path.exists(os.path.join(saving_folder, "notes.pickle"))
        and os.path.getsize(os.path.join(saving_folder, "notes.pickle")) != 0
    ):
    ...
    if posssibilty:
        ...
        while new_input not in choices:
            if not new_input:
                return None
            new_input = input(
                ">>> Enter one of the digits above to choose function: "
            ).strip()
        try:
...
def exit_func():
    ...
    print(f"Yours contacts and notes were saved in such directory:\n{saving_folder}")
...
def show_commands():
    ...
    print("All commands:")
    contact_func = list(
        key for key, value in handler.items() if hasattr(Book, value[0].__name__)
    )
    notes_func = list(
        key for key, value in handler.items() if hasattr(Notes, value[0].__name__)
    )
    other_commands = [
        "show_commands",
        "show_doc",
    ...
    for func in contact_func:
        try:
            table.append(
                [func, notes_func[n], exit_words[n], hello_words[n], other_commands[n]]
            )
            n += 1
        except IndexError:
            try:
                table.append([func, notes_func[n], " ", " ", other_commands[n]])
                n += 1
            except IndexError:
                try:
...
handler = {
    "add_contact": (
        ...
        (name_input, phones_input, email_input, birthday_input, address_input),
        (
            "name",
    ...
    "add_phone": (
        contacts.add_phone,
        (name_input, phones_input),
        (
            "contact`s name for which you want add phones",
            "new phones numbers (max 3 in contact)",
        ),
    ),
    "change_phone": (
        contacts.change_phone,
        (name_input, phones_input, phones_input),
        ("name", "phone number which you want to change", "new phone number"),
    ),
    "remove_phone": (
        contacts.remove_phone,
        (name_input, phones_input),
    ...
    "remove_email": (
        contacts.remove_email,
        (name_input,),
        ("the name for which you want to remove email",),
    ),
    "add_birthday": (
        contacts.add_birthday,
        (name_input, birthday_input),
        ("the name for which you want to add birthday", "the birthday date to add"),
    ),
    "show_birthdays_after": (
        contacts.show_birthdays_after,
        (daysnumber_input,),
        ("days to birthday"),
    ),
    "change_address": (
        contacts.change_address,
        (name_input, address_input),
        ("the name for which you want to change address", "the new address"),
    ),
    "remove_address": (
        contacts.remove_address,
        (name_input,),
        ("the name for which you want to remove address",),
    ),
    "show_all_contacts": (contacts.show_all_contacts,),
    # notes handling
    "add_note": (
        notes.add_note,
        (title_input, keywords_input, notedata_input),
        ("the title", "the keywords (max 3)", "the note data"),
    ),
    "change_note": (
        notes.change_note,
        (title_input, title_input, keywords_input, notedata_input),
        (
    ...
    "show_note": (notes.show_note, (title_input,), ("the title",)),
    "sort_notes_by": (notes.sort_notes_by, (category_input,), ("the category",)),
    "show_notes_with_tag": (notes.show_notes_with_tag, (tag_input,), ("the keyword",)),
    "change_title": (
        notes.change_title,
